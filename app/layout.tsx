        ...
        >
            ...
            >
                ...
                <svg
                    ...
                    viewBox="0 0 100 6"
                >
                    <rect x="0" y="1.5" width="100" height="3" fill="currentColor" />
                </svg>
            </div>
        </div>
    );
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
            ...
            .then(data => {
                if (data.settings) {
                    if (data.settings.navigation) {
                        const modularNavItems = MODULE_MANIFESTS.filter(
                            (m: ModuleManifest) =>
                                MODULE_VIEWS[m.id] &&
                                (data.settings.modules as Record<string, { enabled: boolean }>)?.[m.id]?.enabled ===
                                    true &&
                                !m.hidden
    ...
    const visibleNavItems = Array.from(
        new Map(navItems.filter(item => item.visible && item.id !== 'settings').map(item => [item.id, item])).values()
    );
    ...
    const renderNavItem = (item: NavItem, isMobile = false, onClick?: () => void) => {
        ...
        const isActuallyModular =
            safePath.startsWith('/view/') ||
            managedServicePages.some(p => safePath.startsWith(p)) ||
            !!MODULE_VIEWS[item.id];
        const targetUrl = item.isIframe && !isActuallyModular ? `/view?url=${encodeURIComponent(safePath)}` : safePath;
        ...
        return (
            <Link
                key={item.id}
                href={finalUrl.startsWith('/') || finalUrl.startsWith('http') ? finalUrl : '/'}
            ...
            <body>
                <div
                    ...
                    style={{
                        ...
                        transition:
                            'background-color 0.4s var(--md-sys-motion-easing-standard), color 0.4s var(--md-sys-motion-easing-standard)',
                ...
                >
                    {!isEmbedded && (
                        <nav className="desktop-nav">
                            ...
                            <div style={{ marginTop: 'auto' }}>{settingsItem && renderNavItem(settingsItem)}</div>
                        </nav>
                    )}
                                ...
                                >
                                    <div style={{ flex: 1 }}>
                                        {visibleNavItems.map(item =>
                                            renderNavItem(item, true, () => setIsDrawerOpen(false))
                                        )}
                                    </div>
                                    <div
                                        style={{
                                    ...
                                    >
                                        {settingsItem &&
                                            renderNavItem(settingsItem, true, () => setIsDrawerOpen(false))}
                                    </div>
                                </div>
                            </nav>
                        </>
