...
export async function GET() {
    try {
        ...
        if (gmailRes.ok) {
            ...
            console.error('Gmail API Error:', gmailRes.status);
        }
        ...
        const calendarRes = await fetch(
            `https://www.googleapis.com/calendar/v3/calendars/primary/events?timeMin=${encodedNow}&maxResults=5&singleEvents=true&orderBy=startTime`,
            {
                headers: { Authorization: `Bearer ${token}` },
            }
        );
        ...
        if (calendarRes.ok) {
            ...
            events = (calendarData.items || []).map((event: GoogleEvent) => {
                return {
            ...
            console.error('Calendar API Error:', calendarRes.status);
        }
        ...
        try {
            const tasksRes = await fetch(
                'https://tasks.googleapis.com/tasks/v1/lists/@default/tasks?status=needsAction',
                {
                    headers: { Authorization: `Bearer ${token}` },
                }
            );
