...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    try {
        ...
        const expectedKey = process.env.MODULE_API_KEY;

        // In production, we REQUIRE an API key if one is set in the environment.
        if (process.env.NODE_ENV === 'production' && expectedKey) {
            const providedToken = authHeader?.startsWith('Bearer ') ? authHeader.slice(7) : '';
            if (!providedToken || !timingSafeEqual(providedToken, expectedKey)) {
