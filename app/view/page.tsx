...
import { ExternalLink, ShieldAlert, Loader2, RefreshCw, Globe } from 'lucide-react';
...
function ViewContent() {
    ...
    if (!url) {
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', gap: '24px', backgroundColor: 'var(--md-sys-color-surface)' }}>
                <div style={{ backgroundColor: 'var(--md-sys-color-error-container)', color: 'var(--md-sys-color-error)', padding: '24px', borderRadius: '32px' }}>
                    <ShieldAlert size={48} />
                </div>
                <div style={{ textAlign: 'center' }}>
                    <h2 style={{ fontSize: '24px', fontWeight: 900, margin: '0 0 8px 0' }}>Access Denied</h2>
                    <p style={{ opacity: 0.6, fontSize: '14px' }}>No valid destination URL was provided to the view engine.</p>
                </div>
                <BackButton />
            </div>
        );
        ...
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', gap: '24px', backgroundColor: 'var(--md-sys-color-surface)' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                    ...
                    <p style={{ opacity: 0.6, fontSize: '14px' }}>The provided URL is invalid or uses an unauthorized protocol.</p>
                </div>
                <BackButton />
            </div>
        );
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', backgroundColor: 'var(--md-sys-color-surface)', overflow: 'hidden' }}>
            <div style={{ 
                padding: '8px 24px', 
                height: '64px',
            ...
            <div style={{ flex: 1, position: 'relative', backgroundColor: '#fff' }}>
                {isLoading && (
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 5, backgroundColor: 'var(--md-sys-color-surface)', gap: '16px' }}>
                        <Loader2 size={40} className="animate-spin" color="var(--md-sys-color-primary)" />
                        <div style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.1em', opacity: 0.5 }}>ESTABLISHING SECURE LINK...</div>
                    </div>
                )}
                <iframe 
                    ref={iframeRef}
...
export default function ViewPage() {
    return (
        <Suspense fallback={
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: 'var(--md-sys-color-surface)' }}>
