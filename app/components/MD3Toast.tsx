...
export const MD3Toast = ({ message, type, onClose, duration = 4000 }: MD3ToastProps) => {
    ...
    const getIcon = () => {
        switch (type) {
            case 'success': return <CheckCircle size={20} color="var(--md-sys-color-primary)" />;
            case 'error': return <AlertCircle size={20} color="var(--md-sys-color-error)" />;
            case 'loading': return <RefreshCw size={20} className="spinning" />;
            default: return null;
        }
    };
