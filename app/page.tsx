...
const WIDGETS_DEF: Record<
    string,
    { name: string; icon: React.ElementType; color: string; render: (settings: AppSettings | null) => React.ReactNode }
> = {
    weather: {
        name: 'Weather / Clock',
        icon: CloudRain,
        color: '#a8c7fa',
        render: settings => {
            const W = MODULE_WIDGETS['weather'];
            return W ? <W settings={settings?.weather} /> : null;
        },
    },
    loopia: {
        name: 'Loopia',
    ...
    cloudflare: {
        ...
        color: '#f38020',
        render: () => {
            const W = MODULE_WIDGETS['cloudflare'];
            return W ? <W /> : null;
        },
    },
    unifi: {
    ...
    proxmox: {
        ...
        color: '#e57000',
        render: () => {
            const W = MODULE_WIDGETS['proxmox'];
            return W ? <W /> : null;
        },
    },
    npm: {
    ...
    workspace: {
        ...
        color: '#4285f4',
        render: () => {
            const W = MODULE_WIDGETS['workspace'];
            return W ? <W /> : null;
        },
    },
    ...
    homeassistant: {
        ...
        color: '#03a9f4',
        render: () => {
            const W = MODULE_WIDGETS['homeassistant'];
            return W ? <W /> : null;
        },
    },
    uptimekuma: {
        ...
        color: '#6ad3cb',
        render: () => {
            const W = MODULE_WIDGETS['uptimekuma'];
            return W ? <W /> : null;
        },
    },
    media: {
    ...
    'example-hello': {
        ...
        color: '#9333ea',
        render: () => {
            const W = MODULE_WIDGETS['example-hello'];
            return W ? <W /> : null;
        },
    },
};
...
export default function DashboardHub() {
    ...
    const [widgetVisuals, setWidgetVisuals] = useState<
        Record<string, { theme: string; opacity: number; customColor?: string | null }>
    >({});
    ...
    const [visualsBackup, setVisualsBackup] = useState<Record<
        string,
        { theme: string; opacity: number; customColor?: string | null }
    > | null>(null);
    ...
    const weather = useWeather(settings?.weather || null);
    ...
    const handleRemove = useCallback((rowIdx: number, sectionIdx: number, id: string) => {
        setRows(prev =>
            prev.map((row: DashboardRow, rIdx: number) => {
    ...
    const handleMoveWidget = (fromRowIdx: number, fromSectionIdx: number, toSectionIdx: number, widgetId: string) => {
        setRows(prev =>
            prev.map((row: DashboardRow, rIdx: number) => {
                ...
                const widget = fromSec.layout.find(
                    (w: DashboardLayoutItem) => (typeof w === 'string' ? w : (w as { i: string }).i) === widgetId
                );
                ...
                return {
                    ...
                    sections: row.sections.map((sec: DashboardSection, sIdx: number) => {
                        if (sIdx === fromSectionIdx) {
                            return {
                                ...sec,
                                layout: sec.layout.filter(
                                    (w: DashboardLayoutItem) =>
                                        (typeof w === 'string' ? w : (w as { i: string }).i) !== widgetId
                                ),
                            };
                        }
        ...
        setRows(prev =>
            prev.map((row: DashboardRow, rIdx: number) => {
                ...
                return {
                    ...
                    sections: row.sections.map((sec: DashboardSection, sIdx: number) => {
                        if (sIdx !== sectionIdx) return sec;
                        const index = sec.layout.findIndex(
                            (w: DashboardLayoutItem) =>
                                (typeof w === 'string' ? w : (w as { i: string }).i) === widgetId
                        );
    ...
    const handleExecuteDelete = () => {
        ...
        if (confirmDelete.type === 'row') {
            ...
            setRows(prev =>
                prev.map((r: DashboardRow, ri: number) =>
                    ri === confirmDelete.rowIdx
                        ? {
                              ...r,
                              sections: r.sections.filter(
                                  (_: DashboardSection, si: number) => si !== confirmDelete.sectionIdx
                              ),
                          }
                        : r
                )
    ...
    const handleLayoutChange = useCallback(
        (layout: readonly DashboardLayoutItem[], rowIdx: number, sectionIdx: number) => {
            if (!editMode) return;
    ...
    const handleResizeStop = useCallback(
        (layout: readonly DashboardLayoutItem[], _oldItem: DashboardLayoutItem, newItem: DashboardLayoutItem) => {
    ...
    const dashboardBgStyle: React.CSSProperties =
        appearance.bgType === 'image' && appearance.backgroundImage
            ? {
            ...
            : {
                  backgroundColor:
                      ...
                      appearance.backgroundColor !== '#111318' &&
                      appearance.backgroundColor !== '#1a1c22' &&
                      appearance.backgroundColor !== '#0f1117'
                          ? appearance.backgroundColor
                          : undefined,
                    ...
                    >
                        <div
                            style={{
                                ...
                                display: isMobile && editMode ? 'none' : 'block',
                            }}
                        >
                            <div
                                style={{
                            ...
                            <h1
                                ...
                                style={{
                                    fontSize: isMobile ? 'clamp(28px, 8vw, 36px)' : 'clamp(32px, 6vw, 56px)',
                                    fontWeight: 900,
                                    margin: 0,
                            ...
                            >
                                ...
                                <span
                                    style={{
                                        ...
                                        backgroundImage:
                                            'linear-gradient(to bottom right, var(--md-sys-color-primary), var(--md-sys-color-secondary))',
                                        WebkitBackgroundClip: 'text',
                                        WebkitTextFillColor: 'transparent',
                                    }}
                                >
                                    {appearance.userName || 'Horizon'}
                                </span>
                            </h1>
                        </div>
                                ...
                                >
                                    <button
                                        onClick={() => {
                                            ...
                                            if (loc) {
                                                const safeLoc = sanitizeUrl(loc);
                                                // snyk-ignore: javascript/OR
                                                if (safeLoc.startsWith('http')) {
                                                    window.open(safeLoc, '_blank');
                                                }
                                            }
                                        }}
                                    ...
                                    >
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                            ...
                                            >
                                                ...
                                                settings?.weather?.location.toUpperCase() === 'ABOUT:BLANK'
                                                    ? 'Stockholm'
                                                    : settings.weather.location}
                                            </div>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                ...
                                                >
                                                    {weather.time.getHours().toString().padStart(2, '0')}:
                                                    {weather.time.getMinutes().toString().padStart(2, '0')}
                                                </span>
                                            </div>
                                        </div>
                                    </button>
                                                    ...
                                                    {(() => {
                                                        ...
                                                        if (desc.includes('clear'))
                                                            return (
                                                                <Sun
                                                                    size={20}
                                                                    color="var(--md-color-weather-sun)"
                                                                    opacity={0.9}
                                                                />
                                                            );
                                                        if (desc.includes('rain') || desc.includes('drizzle'))
                                                            return (
                                                                <CloudRain
                                                                    size={20}
                                                                    color="var(--md-color-weather-rain)"
                                                                    opacity={0.9}
                                                                />
                                                            );
                                ...
                                {row.sections.map((section: DashboardSection, sIdx: number) => (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            width:
                                                row.layout === 'split' && !isMobile
                                                    ? `calc(${100 / row.sections.length}% - ${(32 * (row.sections.length - 1)) / row.sections.length}px)`
                                                    : '100%',
                                            minWidth:
                                                row.layout === 'split' && !isMobile
                                                    ? isMobile
                                    ...
                                    >
                                            ...
                                            {editMode && (
                                                <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                                    ...
                                                    <button
                                                        onMouseDown={e => e.stopPropagation()}
                                                        onClick={() =>
                                                            setAddingToSection({ rowIdx: rIdx, sectionIdx: sIdx })
                                                        }
                                        ...
                                        >
                                            <ResponsiveGridLayout
                                                ...
                                                onLayoutChange={l =>
                                                    handleLayoutChange(
                                                        l as unknown as DashboardLayoutItem[],
                                                        rIdx,
                                                        sIdx
                                                    )
                                                }
                                                onResizeStop={(l, o, n) =>
                                                    handleResizeStop(
                                                        l as unknown as DashboardLayoutItem[],
                                                        o as unknown as DashboardLayoutItem,
                                                        n as unknown as DashboardLayoutItem
                                                    )
                                                }
                                            >
                                                {section.layout.map((item: DashboardLayoutItem) => {
                                                    const id =
                                                        typeof item === 'string' ? item : (item as { i: string }).i;
                                                    const isBookmark = id.startsWith('bookmark-');

                                                    const bookmark = isBookmark
                                                        ? bookmarks.find(b => b.id === id.replace('bookmark-', ''))
                                                        : null;
                                                    const wDef =
                                                        isBookmark && bookmark
                                                            ? {
                                                                  ...
                                                                  render: () => (
                                                                      <div
                                                                          onClick={() => {
                                                                              const bUrl = sanitizeUrl(bookmark.url);
                                                                              // snyk-ignore: javascript/OR
                                                                              if (!editMode && bUrl.startsWith('http'))
                                                    ...
                                                    const visuals = widgetVisuals[id] || {
                                                        theme: 'default',
                                                        opacity: 1,
                                                    };
                                                    const accentRgb = hexToRgb(appearance.accentColor || '#3b82f6');
                                                    const customRgb = visuals.customColor
                                                        ? hexToRgb(visuals.customColor)
                                                        : null;
                                                    ...
                                                    const bgStyle =
                                                        visuals.theme === 'glass'
                                                            ...
                                                            : visuals.theme === 'accent'
                                                              ? {
                                                                    background: `rgba(${accentRgb}, ${visuals.opacity})`,
                                                                    color: '#fff',
                                                                }
                                                              : visuals.theme === 'tonal'
                                                                ? {
                                                                      background: `rgba(${accentRgb}, ${visuals.opacity * 0.2})`,
                                                                      color: 'var(--md-sys-color-on-surface)',
                                                                  }
                                                                : visuals.customColor
                                                                  ? {
                                                                        background: `rgba(${customRgb}, ${visuals.opacity})`,
                                                                        color: '#fff',
                                                                    }
                                                                  : {
                                                                        background:
                                                        ...
                                                        <div key={id}>
                                                            <div
                                                                ...
                                                                style={{
                                                                    ...
                                                                    ...bgStyle,
                                                                    borderRadius:
                                                                        'calc(var(--md-sys-shape-corner-extra-large) * 1.2)',
                                                                    position: 'relative',
                                                                    overflow: 'hidden',
                                                                    boxShadow: editMode
                                                                ...
                                                                {editMode && (
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            aria-label="Widget visual settings"
                                                            ...
                                                            {moveTarget?.id === id && (
                                                                <div
                                                                    ...
                                                                    style={{
                                                                        ...
                                                                        border: '1px solid var(--md-sys-color-outline-variant)',
                                                                        animation:
                                                                            'fadeInScale 0.2s cubic-bezier(0.2, 0.0, 0, 1.0)',
                                                                    }}
                                                                >
                                                                    <div
                                                                        style={{
                ...
                >
                    <div
                        style={{
                            position: 'absolute',
                            inset: 0,
                            background: 'rgba(var(--md-sys-color-scrim-rgb), 0.4)',
                                    ...
                                    onClick={() => {
                                        ...
                                        if (editorState.type === 'row') {
                                            ...
                                            if (editorState.rowIdx !== undefined) {
                                                if (editorState.mode === 'add') {
                                                    nr[editorState.rowIdx].sections.push({
                                                        id: `sec-${Date.now()}`,
                                                        name: editorState.name,
                                                        hideTitle: editorState.hideTitle,
                                                        layout: [],
                                                    ...
                                                    nr[editorState.rowIdx].sections[editorState.sectionIdx].name =
                                                        editorState.name;
                                                    nr[editorState.rowIdx].sections[editorState.sectionIdx].hideTitle =
                                                        editorState.hideTitle;
                                                }
                                            }
                                        }
                                        setRows(nr);
                ...
                >
                    ...
                    >
                        ...
                        <div style={{ marginBottom: '40px' }}>
                            ...
                            >
                                {Object.keys(WIDGETS_DEF)
                                    .filter(id => id !== 'example-hello')
                                    .map(id => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                onClick={() => {
                                                    const nr = [...rows];
                                                    const sec =
                                                        nr[addingToSection.rowIdx].sections[addingToSection.sectionIdx];
                                                    const maxY = sec.layout.reduce(
                                                        (max: number, item: DashboardLayoutItem) => {
                ...
                >
                    ...
                    >
                        <div style={{ marginBottom: '16px', textAlign: 'center', color: 'var(--md-sys-color-error)' }}>
                            ...
                            >
                                Delete {confirmDelete.type === 'row' ? 'Row' : 'Section'}?
                            </h3>
                        </div>
                        <p
                            style={{
                        ...
                        >
                            ...
                            {confirmDelete.type === 'row' ? 'row and all its sections' : 'section and all its modules'}.
                        ...
                        <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                            <button
                                onClick={() => setConfirmDelete(null)}
                                className="md3-button-text"
                                style={{ padding: '10px 16px', color: 'var(--md-sys-color-primary)', fontWeight: 500 }}
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="md3-button-text"
                                style={{ padding: '10px 16px', color: 'var(--md-sys-color-error)', fontWeight: 700 }}
                            >
                                Delete
                            </button>
                        </div>
