...
export default function CloudflareView() {
    ...
    const handleSaveRecord = async (record: CFRecord) => {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(
                    resData.error || (resData.errors && resData.errors[0]?.message) || 'Failed to save record'
                );
    ...
    const formatBytes = (bytes: number) => {
        ...
        const i =
            bytesAbs > 0 ? Math.max(0, Math.min(Math.floor(Math.log(bytesAbs) / Math.log(k)), sizes.length - 1)) : 0;
        ...
        return (
            ...
            >
                ...
                <p style={{ opacity: 0.6, maxWidth: '400px' }}>
                    Unable to reach Cloudflare Edge servers. Please verify your API token in settings.
                </p>
                <button
                    onClick={() => refetch()}
                    style={{
                ...
                >
                    ...
                    >
                        <h3 style={{ margin: 0, fontSize: '20px', fontWeight: 800 }}>
                            {isAdding ? 'Add CF Record' : 'Edit CF Record'}
                        </h3>
                        <button
                            onClick={() => {
                                setEditingRecord(null);
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            <label
                                style={{ fontSize: '11px', fontWeight: 900, opacity: 0.5, textTransform: 'uppercase' }}
                            >
                                Type
                            </label>
                            <select
                ...
                >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        ...
                        <div>
                            ...
                            <div
                                style={{ fontSize: '12px', opacity: 0.5, fontWeight: 700, textTransform: 'uppercase' }}
                            >
                        ...
                        {(dnsData?.records || []).map((record: CFRecord) => (
                            ...
                            >
                                <div style={{ display: 'flex', alignItems: 'center', gap: '20px', minWidth: 0 }}>
                                    <div
                                        style={{
                                            backgroundColor: record.proxied
                                                ? 'rgba(243, 128, 32, 0.1)'
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
                                    ...
                                    <div style={{ fontSize: '11px', opacity: 0.4, fontWeight: 700 }}>
                                    ...
                                    <button
                                        onClick={() => {
                                            if (record.id) handleDeleteRecord(record.id);
                                        }}
                                        className="m3-press-effect"
                                        style={{
                                            background: 'none',
        ...
        <div style={{ padding: '0px' }}>
            ...
            >
                {[
                    {
                        label: 'Requests',
                        value: formatNumber(data?.totals?.requests || 0),
                        icon: Zap,
                        color: '#f38020',
                    },
                    {
                        label: 'Threats',
                        value: formatNumber(data?.totals?.threats || 0),
                        icon: Shield,
                        color: '#f87171',
                    },
            ...
            <div
                style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '24px' : '40px' }}
            >
                <div
                    style={{
                        background: 'var(--md-sys-color-surface-container-low)',
                    ...
                    >
                        <div>
                            ...
                            <p style={{ fontSize: '13px', opacity: 0.5, margin: 0 }}>
                                Active network zones and edge configuration
                            </p>
                        </div>
                        <span
                            style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {(data?.domains || []).map((domain: CFDomain) => (
                            <button
                                ...
                                style={{
                                    display: 'grid',
                                    gridTemplateColumns: isMobile ? '1fr 40px' : '2fr 1fr 1fr 1fr 40px',
                                    alignItems: 'center',
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                                    <div
                                        style={{
                                            ...
                                            height: '8px',
                                            borderRadius: '50%',
                                            background: domain.status === 'active' ? '#4ade80' : '#f38020',
                                            boxShadow: domain.status === 'active' ? '0 0 10px #4ade80' : 'none',
                                            flexShrink: 0,
                                        }}
                                    />
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        <span style={{ fontWeight: 800, fontSize: isMobile ? '14px' : '15px' }}>
                                            {domain.name}
                                        </span>
                                        {isMobile && (
                                            <span
                                            ...
                                            >
                                                {domain.plan} Plan • {formatNumber(domain.metrics?.requests || 0)} reqs
                                            </span>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            ...
                                            <span style={{ fontSize: '13px', fontWeight: 700 }}>
                                                {formatNumber(domain.metrics?.requests || 0)} reqs
                                            </span>
                                        </div>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            ...
                                            <span style={{ fontSize: '13px', fontWeight: 700 }}>
                                                {formatBytes(domain.metrics?.bandwidth || 0)}
                                            </span>
                                        </div>
                                    </>
                                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', flex: 1 }}>
                    ...
                    >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            {[
                                ...
                                },
                                {
                                    ...
                                    value: formatNumber(data?.totals?.uniques || 0),
                                    icon: Globe,
                                    color: 'var(--md-sys-color-primary)',
                                },
                            ].map((item, i) => (
                        ...
                        >
                            ...
                            <p style={{ margin: 0, fontSize: '11px', opacity: 0.6, lineHeight: 1.4 }}>
                                Global WAF is monitoring all traffic for SQL injection, bot patterns, and cross-site
                                scripting attacks.
                            </p>
                        </div>
                    </div>
