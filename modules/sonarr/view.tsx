...
import { Video, Download, Database, Loader2, AlertCircle, RefreshCw, Clock } from 'lucide-react';
...
export default function SonarrView() {
    ...
    const fetchData = React.useCallback(async () => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Failed to load Sonarr data');
        } finally {
            setLoading(false);
        }
    }, []);
        ...
        return (
            ...
            >
                ...
                <div style={{ opacity: 0.5, marginTop: '8px' }}>
                    {error || 'Could not connect to the Sonarr API service.'}
                </div>
                <Link
                    href="/"
                    style={{
    ...
    return (
        <div className="md3-master-detail-grid">
            ...
            <aside className="md3-sidebar">
                <div
                    style={{ marginBottom: isMobile ? '0' : '8px', display: 'flex', alignItems: 'center', gap: '16px' }}
                >
                    <BackButton />
                    <div>
                        <h1
                    ...
                    {[
                        {
                            ...
                            icon: Video,
                            color: '#3c9bdc',
                            bgColor: 'rgba(60, 155, 220, 0.15)',
                        },
                        {
                            label: 'Missing',
                            value: data?.stats?.missing || 0,
                            icon: AlertCircle,
                            color: '#f87171',
                        ...
                        {
                            label: 'Storage',
                            value: formatBytes(data?.stats?.totalSize || 0),
                            icon: Database,
                ...
                <div>
                    ...
                    {(data?.queue || []).length > 0 ? (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                            {(data?.queue || []).map((item: SonarrQueueItem) => (
                                <div
                                    key={item.id}
                                    style={{
            ...
            <main style={{ padding: isMobile ? '24px 16px' : '48px', overflowY: 'auto' }}>
                ...
                <div className="md3-poster-grid">
                    {Array.isArray(data?.recent) ? (
                        (data?.recent || []).map((series: SonarrSeries, i: number) => (
                            <div
                                key={series.id}
                                className="md3-poster-card m3-press-effect"
                                style={{
                                ...
                                <div style={{ width: '100%', height: '100%', position: 'relative' }}>
                                    <Image
                                        ...
                                        onError={e => {
                                            ...
                                            target.src =
                                                'https://raw.githubusercontent.com/Sonarr/Sonarr/develop/src/Sonarr.GUI/Content/Images/logo.png';
                                        }}
                                        unoptimized
                                    />
                                    <div
                                    ...
                                    <div style={{ position: 'absolute', bottom: '10px', left: '10px', right: '10px' }}>
                                        ...
                                        <div
                                            style={{ fontSize: '8px', color: 'rgba(255,255,255,0.6)', fontWeight: 700 }}
                                        >
                                            {series.year}
                                        </div>
                                    </div>
