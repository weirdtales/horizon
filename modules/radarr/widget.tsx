...
export default function RadarrWidget() {
    const { data, loading, error } = useService<{
        queue: { id: string | number; title: string; progress: number }[];
        stats: { total: number; missing: number; totalSize: number };
    }>('radarr');
    ...
    if (loading)
        return (
            ...
            >
                ...
                <div style={{ fontSize: '10px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                    Scanning Movies...
                </div>
            </div>
        );
        ...
        >
            <div
                style={{
                    ...
                    right: 0,
                    height: '40%',
                    background: 'linear-gradient(to bottom, rgba(255, 193, 7, 0.08), transparent)',
                    pointerEvents: 'none',
                }}
            />
            ...
            >
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    <div
                        style={{
                            backgroundColor: 'color-mix(in srgb, #ffc107 15%, transparent)',
                            color: '#ffc107',
                            padding: '10px',
                            borderRadius: '14px',
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '16px' }}>
                {hasQueue ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {data.queue
                            .slice(0, 2)
                            .map((item: { id: string | number; title: string; progress: number }) => (
                                <div key={item.id}>
                                    <div
                                        style={{
                                            display: 'flex',
                                    ...
                                    >
                                        ...
                                        <span style={{ fontSize: '11px', fontWeight: 900, opacity: 0.6 }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                        ...
                        >
                            ...
                            <div
                                style={{ fontSize: '10px', fontWeight: 900, opacity: 0.5, textTransform: 'uppercase' }}
                            >
                                Movies
                            </div>
                        </div>
                        ...
                        >
                            <div
                                style={{
                                    ...
                                    color: (data.stats?.missing ?? 0) > 0 ? '#ffc107' : 'inherit',
                                }}
                            >
